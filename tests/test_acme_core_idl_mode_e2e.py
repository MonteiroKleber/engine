"""E2E (HTTP) proof for ACME Core in ENGINE_API_MODE=idl + ENGINE_AUTH_MODE=strict.

Hard-gate test referenced by docs/specs/migracao/03-acme-core-idl-mode/spec.md.
"""
...
def test_acme_core_flow_strict_idl_mode(tmp_path: Path, monkeypatch):
    app = _make_app(monkeypatch, tmp_path, Path("bundles/acme_core").resolve())

    with TestClient(app) as client:
        institution_id, admin_key = _create_institution_and_admin_key(client)
        operator_token = _create_actor_token(client, institution_id, admin_key, "operator")
        manager_token = _create_actor_token(client, institution_id, admin_key, "manager")

        resp = client.post(
            "/finance/expenses",
            headers={"X-Institution-Id": institution_id, "X-Actor-Token": operator_token},
            json={"amount": 100, "description": "acme"},
        )
        ...
        # Operator cannot decide approvals
        resp = client.post(
