"""E2E (HTTP) proof for multi-pilot in ENGINE_API_MODE=idl + ENGINE_AUTH_MODE=strict.

Hard-gate test referenced by docs/specs/migracao/04-multi-pilot-multi-dept-idl-mode/spec.md.
"""
...
def test_multi_dept_finance_and_support_routes(tmp_path: Path, monkeypatch):
    app = _make_app(monkeypatch, tmp_path, Path("bundles/multi-pilot").resolve())

    with TestClient(app) as client:
        institution_id, admin_key = _create_institution_and_admin_key(client)

        finance_analyst = _create_actor_token(client, institution_id, admin_key, "analyst")
        finance_manager = _create_actor_token(client, institution_id, admin_key, "manager")
        support_agent = _create_actor_token(client, institution_id, admin_key, "agent")

        # Finance dept: /d/finance + /finance/expenses
        resp = client.post(
            "/d/finance/finance/expenses",
            headers={"X-Institution-Id": institution_id, "X-Actor-Token": finance_analyst},
            json={"amount": 100, "description": "multi finance"},
        )
        ...
        # Approve as manager
        resp = client.post(
            f"/d/finance/approvals/{approval_id}/decide",
            headers={"X-Institution-Id": institution_id, "X-Actor-Token": finance_manager},
            json={"decision": "approve"},
        )
        assert resp.status_code == 200, resp.text
        assert resp.json()["case_status"] == "COMMITTED"

        # Support dept: /d/support + /support/tickets (no approval requirement asserted here)
        resp = client.post(
            "/d/support/support/tickets",
            headers={"X-Institution-Id": institution_id, "X-Actor-Token": support_agent},
            json={"subject": "help", "description": "ticket"},
        )
        assert resp.status_code in (200, 201, 202), resp.text


def test_multi_dept_missing_token_is_401(tmp_path: Path, monkeypatch):
    app = _make_app(monkeypatch, tmp_path, Path("bundles/multi-pilot").resolve())
    with TestClient(app) as client:
