"""Smoke test: PROD + STRICT + IDL must boot and serve a minimal flow.

Hard-gate test referenced by docs/specs/migracao/06-prod-strict-idl-baseline/spec.md.
"""
...
def _make_app(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setenv("ENGINE_BUNDLE_PATH", str(Path("bundles/finance-pilot").resolve()))
...
def test_prod_strict_idl_boot_health_and_console(tmp_path: Path, monkeypatch):
...
def test_prod_strict_idl_finance_flow(tmp_path: Path, monkeypatch):
    app = _make_app(monkeypatch, tmp_path)
    with TestClient(app) as client:
        ...
        resp = client.post(
            ...
            json={"amount": 10, "description": "prod smoke"},
        )
...
def test_prod_strict_rejects_spoof_headers(tmp_path: Path, monkeypatch):
    app = _make_app(monkeypatch, tmp_path)
    with TestClient(app) as client:
        ...
        resp = client.post(
            ...
            json={"amount": 1, "description": "spoof"},
        )
